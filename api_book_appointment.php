...
try {
    ...
    $stmt = $connect->prepare("INSERT INTO appointment (parent_id, child_id, payment_id, specialist_id, status, type, comment, intake_data, intake_completed, scheduled_at) VALUES (?, ?, ?, ?, 'Scheduled', ?, ?, ?, ?, ?)");
    $stmt->execute([$parentId, $childId, $paymentId, $specialistId, $type, $comment, $intakeJson, $intakeCompleted, $scheduledDateTime]);
