...
if (isset($_GET['ajax']) || (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest')) {
    ...
    if ($method === 'GET' && $action === 'get_profile') {
        $stmt = $connect->prepare("
            SELECT u.user_id, u.first_name, u.last_name, u.email, u.status,
                   ...
                   s.bio, s.phone AS phone, s.clinic_id,
                   c.clinic_name, c.location AS clinic_location,
                   ob.focus_areas, ob.working_days, ob.start_time, ob.end_time,
                   ob.consultation_types, ob.goals, ob.certifications AS onboarding_certifications
            FROM users u
            INNER JOIN specialist s ON u.user_id = s.specialist_id
            LEFT JOIN clinic c ON s.clinic_id = c.clinic_id
            LEFT JOIN doctor_onboarding ob ON ob.doctor_id = u.user_id
            WHERE u.user_id = :uid
        ");
    ...
    if ($method === 'POST' && $action === 'save_profile') {
        ...
        try {
            ...
            ")->execute([':fn' => $first_name, ':ln' => $last_name, ':spec' => $spec, ':exp' => $exp, ':cert' => $cert, ':bio' => $bio, ':phone' => $phone, ':sid' => $doctor_id]);

            // Also update doctor_onboarding if row exists
            $obCheck = $connect->prepare("SELECT id FROM doctor_onboarding WHERE doctor_id = :did LIMIT 1");
            $obCheck->execute([':did' => $doctor_id]);
            if ($obCheck->fetch()) {
                $connect->prepare("
                    UPDATE doctor_onboarding SET specialization = :spec, experience_years = :exp, certifications = :cert
                    WHERE doctor_id = :did
                ")->execute([':spec' => $spec, ':exp' => $exp, ':cert' => $cert, ':did' => $doctor_id]);
            }
                            ...
                            <div class="dr-form-grid">
                                ...
                                <div class="dr-form-group full-width">
                                    ...
                                    <textarea id="dr-bio" class="dr-form-input dr-form-textarea"
                                        placeholder="Write a short bio about your practice and expertise..."><?php echo htmlspecialchars($doctor['bio'] ?? ''); ?></textarea>
                                </div>
                            </div>
                        </div>
